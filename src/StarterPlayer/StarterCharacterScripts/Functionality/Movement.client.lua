-- CREDIT TO: pdnghiaqoi
-- Additions made by EPIXPLODE
local player
local character
local collider
local camera
...
function init(Player, Camera, Input)
	...
	airAccelerate 			= 10000000000
	airMaxSpeed 			= 3
	groundAccelerate 		= 250
	groundMaxVelocity 		= 14.5
	friction			 	= 9
	playerTorsoToGround 	= 3
	movementStickDistance 	= .7
	jumpVelocity 			= 43
...
local spacing = false
local spacecooldown = false
game:GetService("UserInputService").InputBegan:Connect(function(i, g)
	if i.KeyCode == Enum.KeyCode.Space then
		wait(.01)
		spacing = true
		if not spacecooldown then
			spacecooldown = true
			wait(.25)
			spacecooldown = false
		end
...
game:GetService("UserInputService").InputEnded:Connect(function(i, g)
	game:GetService("UserInputService").InputBegan:Connect(function(i, g)
		if i.KeyCode == Enum.KeyCode.Space then
			if spacing then spacing = false end
		end
	end)
end)
...
function update(deltaTime)
	...
	if playerGrounded and(input["Space"] or jumping) and not spacing and not spacecooldown then
		jumping = true
	else
		jumping = false
	end
...
function findCollisionRay()
	...
	local ignoreList = {character, camera, spawns}
	local rays = {
...
function jump(vel)
	...
	local x = math.abs(collider.Velocity.x)
	local z = math.abs(collider.Velocity.y)
	local y = math.abs(collider.Velocity.z)
	
	collider.Velocity = Vector3.new(collider.Velocity.x, vel, collider.Velocity.z)
	
	air()
	--game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("Sound"):WaitForChild("Walk"):FireServer(false)
	--game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("Sound"):WaitForChild("Jump"):FireServer(true)
end
...
function air()
	...
	local running = game:GetService("Players").LocalPlayer.Character:WaitForChild("HumanoidRootPart"):WaitForChild("Running")
...
function run(hitPosition)
	...
	--	game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("Sound"):WaitForChild("Walk"):FireServer(true)
	else
	--	game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("Sound"):WaitForChild("Walk"):FireServer(false)
	end
